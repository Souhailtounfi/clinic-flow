...
function PrescriptionsPage() {
  ...
  const addMedicine = (m: string) => {
    ...
    setQ("");
    toast.success(`Added ${m}`);
  };

  return (
    <AppShell
      title="Prescription editor"
      subtitle={`${patient.firstName} ${patient.lastName} · ${new Date().toLocaleDateString("en-GB", { dateStyle: "medium" })}`}
      actions={
        <>
          <Button variant="outline" className="rounded-lg" onClick={() => toast("Preparing preview…")}><Printer className="mr-2 h-4 w-4" /> Print</Button>
          <Button className="rounded-lg" onClick={() => toast.success("PDF downloaded")}><Download className="mr-2 h-4 w-4" /> Download PDF</Button>
        </>
      }
    >
      <div className="grid gap-4 lg:grid-cols-[1fr_420px]">
        <div className="space-y-4">
          <Card className="rounded-2xl border-border/60 shadow-sm">
            <CardContent className="p-5">
              ...
              <div className="relative mt-2">
                ...
                <Input placeholder="Search medicine…" value={q} onChange={(e) => setQ(e.target.value)} className="h-11 pl-9" />
              ...
              <div className="mt-4">
                <div className="mb-2 text-[11px] font-semibold uppercase text-muted-foreground">Favorites</div>
              ...
              <div className="mt-4">
                <div className="mb-2 text-[11px] font-semibold uppercase text-muted-foreground">Recently prescribed</div>
                <div className="flex flex-wrap gap-1.5">
          ...
          <Card className="rounded-2xl border-border/60 shadow-sm">
            <CardContent className="p-5">
              <div className="mb-3 text-sm font-semibold">Medicines ({items.length})</div>
                ...
                {items.map((it, i) => (
                  <div key={i} className="rounded-xl border p-3">
                    ...
                    <div className="mt-2 grid gap-2 sm:grid-cols-4">
                      <Field label="Dosage" value={it.dosage} onChange={(v) => update(i, "dosage", v)} />
                      <Field label="Frequency" value={it.frequency} onChange={(v) => update(i, "frequency", v)} />
                      <Field label="Duration" value={it.duration} onChange={(v) => update(i, "duration", v)} />
                      <Field label="Instructions" value={it.instructions ?? ""} onChange={(v) => update(i, "instructions", v)} />
                    </div>
                  </div>
                ))}
                {items.length === 0 && <div className="py-6 text-center text-sm text-muted-foreground">No medicines added yet.</div>}
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="rounded-2xl border-border/60 shadow-sm">
          <CardContent className="p-6">
            ...
            <div className="text-[11px] font-semibold uppercase text-muted-foreground">Doctor</div>
            ...
            <div className="mt-4 text-[11px] font-semibold uppercase text-muted-foreground">Patient</div>
            <div className="text-sm font-medium">{patient.firstName} {patient.lastName}</div>
            <div className="text-[11px] text-muted-foreground">{patient.age} y/o · {patient.gender === "F" ? "Female" : "Male"}</div>

            <div className="mt-5 border-t pt-4">
              <div className="text-[11px] font-semibold uppercase text-muted-foreground">Prescription</div>
            ...
            <div className="mt-6 border-t pt-4 text-right">
              <div className="text-[11px] text-muted-foreground">Signature</div>
      ...
      <Card className="mt-6 rounded-2xl border-border/60 shadow-sm">
        <CardContent className="p-5">
          <div className="mb-3 text-sm font-semibold">Prescription history</div>
          <div className="divide-y">
            {prescriptions.slice(0, 6).map((r) => (
              <div key={r.id} className="flex items-center gap-3 py-3">
                ...
                <Button size="sm" variant="outline">Reuse</Button>
              </div>
            ))}
          </div>
        </CardContent>
