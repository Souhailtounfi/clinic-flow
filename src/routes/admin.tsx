...
function AdminPage() {
  ...
  return (
    <AppShell title="Super admin" subtitle="Platform overview across all clinics">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <AdminStat icon={Building2} label="Active clinics" value={active.toString()} delta={`${inactive} inactive`} />
        <AdminStat icon={Users} label="Total patients" value={totalPatients.toLocaleString()} delta="+312 this week" />
        <AdminStat icon={TrendingUp} label="MRR" value={`${totalRevenue.toLocaleString()} MAD`} delta="+22%" />
        <AdminStat icon={Activity} label="Uptime" value="99.98%" delta="30d" />
      </div>

      <Card className="mt-6 rounded-2xl border-border/60 shadow-sm">
        <CardContent className="p-5">
          <div className="mb-4 text-sm font-semibold">Platform revenue</div>
          <div className="h-64">
      ...
      <Card className="mt-4 overflow-hidden rounded-2xl border-border/60 shadow-sm">
        <div className="flex items-center justify-between border-b p-4">
          <div className="text-sm font-semibold">Clinics ({clinics.length})</div>
          <Button size="sm" onClick={() => toast.success("Invite sent")}>Invite clinic</Button>
        </div>
                ...
                <TableRow key={c.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <div><div className="text-sm font-medium">{c.name}</div><div className="text-[11px] text-muted-foreground">Since {c.since}</div></div>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    {c.status === "active"
                      ? <Badge className="rounded-full bg-[oklch(0.94_0.06_155)] text-[oklch(0.35_0.12_155)] hover:bg-[oklch(0.94_0.06_155)]">Active</Badge>
                      : <Badge variant="secondary" className="rounded-full">Inactive</Badge>}
                  </TableCell>
                  <TableCell className="text-right"><Button size="sm" variant="outline" onClick={() => toast("Managing clinic…")}>Manage</Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
