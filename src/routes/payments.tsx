...
function PaymentsPage() {
  ...
  return (
    <AppShell
      title="Payments"
      subtitle="Cash, card & transfer · MAD"
      actions={<Button className="rounded-lg" onClick={() => toast.success("Payment recorded")}><Plus className="mr-2 h-4 w-4" /> Record payment</Button>}
    >
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <StatCard label="Revenue this month" value={`${paid.toLocaleString()} MAD`} delta="+18%" tone="success" />
        <StatCard label="Outstanding" value={`${outstanding.toLocaleString()} MAD`} delta={`${payments.filter(p => p.status === "unpaid").length} invoices`} tone="warning" />
        <StatCard label="Average ticket" value={`${Math.round(paid / Math.max(payments.filter(p => p.status === "paid").length, 1))} MAD`} delta="—" tone="primary" />
      </div>
              ...
              {filtered.slice(0, 20).map((p) => (
                <TableRow key={p.id} className="hover:bg-secondary/30">
                  ...
                  <TableCell className="text-right">
                    {p.status === "unpaid"
                      ? <Button size="sm" variant="outline" onClick={() => toast.success("Marked as paid")}>Mark paid</Button>
                      : <Button size="sm" variant="ghost" className="text-xs text-muted-foreground">Receipt <ArrowUpRight className="ml-1 h-3 w-3" /></Button>}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
